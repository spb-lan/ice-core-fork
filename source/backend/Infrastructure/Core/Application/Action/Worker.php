...
namespace Ifacesoft\Ice\Core\Infrastructure\Core\Application\Action;

use Ice\Core\Action_Worker;
...
abstract class Worker extends Action
{
    /**
     * @inheritDoc
     */
    ...
    private function dispatch(array $dispatchWorker, array $params)
    {
        ...
        foreach ($tasks as $task) {
            ...
            $bg = $worker['bg'] === null ? (int)$worker['max'] > 1 && $worker['tasks'] > 1 : (bool)$worker['bg'];
...
//            $this->getLogger()->info($taskCount . '/' . $totalTasks . ': #' . $hash . ' ' . Type_String::printR($task));
            ...
            try {
                $class::call(['workerKey' => $workerKey, 'hash' => $hash], 0, $bg);
            } catch (\Exception $e) {
